...
```{r annual_incidence}
# XXXX what about the first dispensing per pat_id?
#incident dispensings;
ADHD_1 <- ADHD %>%
  group_by(pat_id) %>%
  arrange(pat_id, supp_date) %>%
  filter(row_number() == 1) %>%
  filter(supp_date > as.Date(min(ADHD$supp_date) + months(17))) %>% # at least 18-month period look back
  filter(age > 17) %>%
...
```
## Part 2: Interuppted time series analysis of ADHD medicines from February 2016 to January 2022.
...
plot_stl <- ts %>% filter(date < yearmonth(change_date)) %>%
  fill_gaps() %>% model(STL = STL(log(rate)))
...
```{r plot_raw_season, results= 'asis', fig.height=14, fig.width=30, eval = TRUE, include = FALSE}
...
```{r residual_plots, eval = TRUE, include = FALSE}
#######################################################################################
...
```
```{ r print_residual_check_table}
#print table in word knit;
ft
```

Plotting figure 1 and estimates of change with 95% confidence intervals

```{r seasonal_model_plot, echo = TRUE, eval = TRUE, include = FALSE}
...
ggsave(
  filename = paste0(Sys.Date(), "Figure 1. Monthly rate of ADHD medicine dispensings per 1000 Australian adults, observed (dotted) and predicted (solid line) trends from 1 February 2016 to 31 January 2022. A", model, "by medicine and sex.tiff"),
  plot = plot,
  device = NULL,
  scale = 1,
  # width = 4000,
...
mean_data %>%
  select(-data) %>%
  flextable() %>%
  add_header_lines("Mean monthly dispensings in year prior to and following intervention")
