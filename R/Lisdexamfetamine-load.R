...
for(yr in c("2015", "2016", "2017", "2018", "2019", "2020", "2021", "2022")) {
  ...
  pbs <- read_sas(paste0(PBSDIR, "y", yr, "supply.sas7bdat"),
                  ...
                  col_select=c("SUPP_DATE", "SCRIPTS", "ATC_CODE",
                               ...
                               "PRESC_ID", "AUTH_FORM_ID", "STREAM_AUTH_CODE", "PRESC_DATE"
                                 ))
...
check <- cbind(ADHD_auth, duplicated(ADHD_auth, by = c("PAT_ID",  "ITEM_CODE", "SUPP_DATE"))) %>% 
  filter(V2==TRUE) %>% arrange(PAT_ID, SUPP_DATE, ITEM_CODE)

#create new rstr_num : some streamline auth codes where restr_num missing
#some cases where rstr_num not equal to stream_code
 check2 <- ADHD_auth[, REST_AUTH_NUM:= fifelse(is.na(RSTR_NUM), STREAM_AUTH_CODE, RSTR_NUM)] %>% 
.[, .N, by = .(ATC_CODE, REST_AUTH_NUM)] %>% arrange(ATC_CODE, REST_AUTH_NUM) 
